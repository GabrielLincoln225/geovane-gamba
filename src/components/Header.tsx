    ...
    <>
      ...
      >
        <div className="max-w-[1440px] mx-auto px-5 sm:px-8 md:px-12 lg:px-16 h-16 sm:h-20 flex items-center justify-between">
          ...
          >
            <span className="font-condensed font-black text-lg sm:text-2xl uppercase tracking-tight text-white group-hover:text-white/90 transition-colors">
              GEOVANE GAMBA
            </span>
            <span className="font-condensed font-bold text-lg sm:text-2xl text-wd-orange">
              20444
            </span>
          </a>
          ...
          <div className="flex md:hidden items-center">
            <button
              ...
              className="p-2.5 -mr-2 text-white/80 hover:text-white transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
            >
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[60] flex justify-end md:hidden">
          ...
          {/* Sheet Panel */}
          <div className="relative w-4/5 max-w-xs h-full bg-wd-deep-blue border-l border-white/10 p-6 pt-[max(1.5rem,env(safe-area-inset-top))] pb-[max(1.5rem,env(safe-area-inset-bottom))] flex flex-col justify-between shadow-2xl animate-in slide-in-from-right duration-300">
            <div>
              {/* Header inside sheet */}
              <div className="flex items-center justify-between pb-5 border-b border-white/10">
                ...
                <button
                  ...
                  className="p-2 -mr-2 text-white/70 hover:text-white min-w-[40px] min-h-[40px] flex items-center justify-center"
                >
                ...
                {NAV_LINKS.map(({ label, href }) => {
                  ...
                  return (
                    <a
                      ...
                      href={href}
                      onClick={(e) => handleScrollTo(e, href)}
                      className={`flex items-center justify-between font-sans text-lg font-semibold py-3 px-3 rounded transition-colors ${
                        isActive
                          ? "text-wd-orange bg-white/5"
                          : "text-white/80 hover:text-white active:bg-white/5"
                      }`}
                    >
                      <span>{label}</span>
                      {isActive && <span className="w-2 h-2 rounded-full bg-wd-orange" />}
                    </a>
                  )
                })}
              </nav>
            ...
            <div className="pt-6 border-t border-white/10 flex flex-col gap-3">
              <button
                ...
                className="w-full flex items-center justify-center gap-2.5 bg-wd-orange hover:bg-[#d8580f] active:bg-[#c44e0c] text-white py-3.5 rounded-[0.25rem] font-sans font-semibold text-sm uppercase tracking-wider shadow-lg active:scale-98 min-h-[46px]"
              >
