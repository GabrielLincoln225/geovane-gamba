...
export const MobileBar: React.FC = () => {
  ...
  return (
    <aside
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-wd-deep-blue/95 backdrop-blur-md border-t border-white/10 px-6 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-2xl transition-shadow"
      style={{ transform: "translateY(120%)", opacity: 0 }}
    >
      <div className="flex items-center justify-between gap-4">
        {/* Left: Vote 20444 */}
        <div className="flex items-center gap-1.5">
          <span className="font-condensed font-bold uppercase text-lg text-white tracking-tight">
            Vote
          </span>
          <span className="font-condensed font-black text-xl text-wd-orange tracking-tight">
            20444
          </span>
        </div>
        ...
        <button
          type="button"
          onClick={handleShare}
          className="inline-flex items-center gap-2 bg-wd-orange hover:bg-[#d8580f] text-white px-4 py-2 rounded-[0.25rem] font-sans font-semibold text-xs uppercase tracking-wider transition-colors duration-200 cursor-pointer shadow-md active:scale-95"
        >
